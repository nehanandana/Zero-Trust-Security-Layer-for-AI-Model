...
def log_to_database(user_id, prompt, risk, action):
    """Save malicious prompts to database for admin review"""
    try:
        conn = sqlite3.connect('users.db', timeout=20)
        conn.execute(
            "INSERT INTO malicious_logs (user_id, prompt, risk, action) VALUES (?, ?, ?, ?)",
            (user_id, prompt, risk, action)
        )
        conn.commit()
        conn.close()
...
def log_prompt(user_id, prompt, risk):
    ...
    # Write to file log safely
    with log_lock:
        with open(log_path, "a") as f:
            f.write(f"{datetime.datetime.now()} | {user_id} | {risk} | {action} | {prompt}\n")

    # Log malicious to database for admin review
    if risk == "Malicious":
        log_to_database(user_id, prompt, risk, action)